...
public:
	...
	public:
		...
		void setSweepParameters(int, int, int, bool, bool, double);
...
* @return: 		    None
*************************************************************************************************/
void IA::setSweepParameters(int begin_Freq, int f_Step, int nSamples, bool r_Flag, bool cal_Flag, double calibration_imp)
{
	SParam.start_Freq = begin_Freq;
	SParam.freq_Step = f_Step;
	SParam.num_sweep_Samples = nSamples;
...
void IA::PerformFrequencySweep()
{
	...
	setNumSamples(SParam.num_sweep_Samples);

	setSettlingTime(10, 3);

	if(IA_DATA.v_pp_code){ //Range needs to be set before performing a freq sweep
		...
		for(int num = 0; num < SParam.num_sweep_Samples; num++)
		{
			...
			if(SParam.calibration_Flag)
			{
				...
				if(IA_DATA.gf_option == 0) //20 Ohm Feed Back Resistor
				{
...
* @return:		 none
******************************************************************/
void IA::SetCalibrationImp(double calibrationIMP)
{
	SParam.calibration_imp = calibrationIMP;
}
