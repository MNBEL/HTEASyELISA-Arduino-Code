...
void IA::setControlRegister(unsigned int function)
{
	...
	setV_pp(IA_DATA.v_pp_code); // 1 200, 2 400, 3 1000, 4 2000 mV
															// scaled to VDD

	setPGA_gain(IA_DATA.pga_gain); // 1 x1, 5 x5

	setFBR(IA_DATA.fbr_code); // 0 20, 1 100K
...
void IA::MeasureGainFactor(float calibration_imp){
	if(check_v_pp_code_set()){
		...
		if(IA_DATA.fbr_code == 1) //20 Ohm Feed Back Resistor
		{
