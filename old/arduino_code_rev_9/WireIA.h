...
public:
	...
	public:
		...
		void setSweepParameters(unsigned long, int, int);
		...
		void SetCalibrationImp(double calibrationIMP);
...
{
	FBR_select_pin = fbr_select_pin;
	pinMode(fbr_select_pin, OUTPUT);
	IA_init();
...
//	imagData 		= {0};
	...
	SParam.calibration_imp = 100000;

	IA_DATA.gf_addr = 0;
	IA_DATA.gf_value_0 = 0.0;
	IA_DATA.gf_value_1 = 0.0;
	IA_DATA.rVAL = 0;
	IA_DATA.iVAL = 0;
	IA_DATA.v_pp_code = 1;
	IA_DATA.pga_gain_code = 1;
	IA_DATA.fbr_code = 0;
...
void IA::EEPROMReadGF()
{
	...
	if(addr == 20)
	{
		rptr = (char *)&IA_DATA.gf_value_0;
		for(i = 0; i < sizeof(IA_DATA.gf_value_0); i++ )
		{
	...
	{
		rptr = (char *)&IA_DATA.gf_value_1;
		for(i = 0; i < sizeof(IA_DATA.gf_value_1); i++ )
		{
		rptr[addr] = EEPROM.read(addr);
		addr++;
		}
...
void IA::setFBR(int option)
{
	...
	{
		case 0: // 20 ohm
			digitalWrite(FBR_select_pin, HIGH);
			FBR_val = 20;
			break;
		case 1: // 100K ohm
			digitalWrite(FBR_select_pin, LOW);
			FBR_val = 100000;
			break;
		default:
			break;
...
void IA::setSweepParameters(unsigned long begin_Freq, int f_Step, int nSteps)
{
...
void IA::setRangeParameters(int pga_gain_code, int fbr_code, int v_pp_code){
	IA_DATA.v_pp_code = v_pp_code; // 1 for 200, 2 for 400, 3 for 1000, 4 for 2000
																// these are scaled by VDD of course
	IA_DATA.pga_gain_code = pga_gain_code; // 1 for x1, 5 for x5
	IA_DATA.fbr_code = fbr_code; // 0 for 20, 1 for 100K
}
/*********************************************************************
...
void IA::BeginFrequencySweep()
{
	...
	setSettlingTime(10, 1); // Should this be dynamic?
	// delay(25);
...
void IA::MeasureGainFactor(float calibration_imp){
	if(check_v_pp_code_set()){
		SParam.calibration_imp = calibration_imp;
		BeginFrequencySweep();
		CalculateGainFactor();
		IA_DATA.SYSPhase = 0; /// need to 0!, is this the right place to do it?
		Phase();
		IA_DATA.SYSPhase = IA_DATA.ph_angle;

		if(IA_DATA.fbr_code == 0) //20 Ohm Feed Back Resistor
		{
			// twoPT_GF[num] = IA_DATA.gf_value_20;
			IA_DATA.gf_addr = 20;
		}
		else{ //100K ohm resistor
...
void IA::CalculateGainFactor()
{
	double gainFactor;
	double magnitude = 0.0;
	magnitude = sqrt(((float)IA_DATA.rVAL * (float)IA_DATA.rVAL) + ((float)IA_DATA.iVAL * (float)IA_DATA.iVAL));

	if(IA_DATA.fbr_code == 0){
		IA_DATA.gf_value_0 = 1 / (magnitude * SParam.calibration_imp);
	}
	else{
		IA_DATA.gf_value_1 = 1 / (magnitude * SParam.calibration_imp);
	}
}
/*****************************************************************
* @function: 	 Impedance
...
void IA::Impedance()
{
	double Z = 0.0;
	double magnitude = 0.0;
	// Serial.print(IA_DATA.rVAL);
	// Serial.print(" ");
	// Serial.print(IA_DATA.iVAL);
	magnitude = sqrt(((float)IA_DATA.rVAL * (float)IA_DATA.rVAL) + ((float)IA_DATA.iVAL * (float)IA_DATA.iVAL));
	Serial.print("in Impedance() magnitude: ");
	Serial.println(magnitude);
	// Serial.print(" ");
	// Serial.print(magnitude);

	if(IA_DATA.fbr_code == 0){
		IA_DATA.imp = 1 / (IA_DATA.gf_value_0 * magnitude);
	}
	else{
		IA_DATA.imp = 1 / (IA_DATA.gf_value_1 * magnitude);
	}
	// Serial.print(" ");
	// Serial.print(IA_DATA.gf_value_0, 10);
	// Serial.print(" ");
	// Serial.println(IA_DATA.gf_value_1, 10);
	// Serial.println(IA_DATA.imp);
}
...
{
	double ph;
	IA_DATA.ph_angle = atan2(IA_DATA.iVAL, IA_DATA.rVAL) - IA_DATA.SYSPhase;
}
...
//	long imaginary_data;
//
//	real_data_7_0 = readData_AD5933(real_data_register_7_0);
//	real_data_15_8 = readData_AD5933(real_data_register_15_8);
//	imaginary_data_7_0 = readData_AD5933(imaginary_data_register_7_0);
//	imaginary_data_15_8 = readData_AD5933(imaginary_data_register_15_8);
//	real_data = (((long)real_data_15_8) << 8) + (int)real_data_7_0;
//	imaginary_data = (((long)imaginary_data_15_8) << 8) + (int)imaginary_data_7_0;
//	// Serial.print(real_data, BIN);
//	// Serial.print(" ");
//	// Serial.print(imaginary_data, BIN);
//	// Serial.print(" ");
//	///
...
double IA::getGF()
{
	...
	{
		case 20: Serial.println(IA_DATA.gf_value_0, 10);
				 gain_value = IA_DATA.gf_value_0;
			break;
		case 100: Serial.println(IA_DATA.gf_value_1, 10);
				  gain_value = IA_DATA.gf_value_1;							//changed to .gf_value_100K from .gf_value_20 --11/06/2015 JColvin
			break;
		default:
			break;
	}
